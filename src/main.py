...
def calculate_appointment_price(num_of_minutes):
    ...
    if 30 <= num_of_minutes <= 120:
        return 4 * (num_of_minutes / 60)
    elif 150 <= num_of_minutes <= 240:
        return 3.5 * (num_of_minutes / 60)
    else:
        return 3 * (num_of_minutes / 60)
...
def verify_login(username, password):
    return any(
        ...
        for row in cursor.execute(
            "select user_name,password from clients where user_id_pk = "
            + "(select user_id_pk from clients where user_name = '"
            + username
            + "')"
        )
    )
...
def reservation():
    ...
    computers_list = [row[0] for row in cursor.execute(command)]
...
def check_reservation():
    ...
    if beginning_hour < datetime.now():
        ...
        return redirect(url_for("reservation"))
    elif not beginning_hour.minute in [0, 30]:
        g_error_msg = "Beginning hour must be either o'clock or half."
        return redirect(url_for("reservation"))
    elif not ending_hour.minute in [0, 30]:
...
def employee_login():
    employees = [
        row[0] + " " + row[1] + " "
        for row in cursor.execute(
            "select first_name,last_name from ic_employees"
        )
    ]
